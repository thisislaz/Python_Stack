#INPUTS AND OUTPUTS
    #what all coding languages have in common is the abulity to interpret code and agive a result(OutPut)
    #many times we write our code to work with a certain set of data, but the data could be diferent in different situations
    #in this case, we set up yo pass an input to our code, sot he interpreter can work with that data.
    #"input()" function in python allows usto interact with the bash window
...
print(f'Your favorite color is: {favorite_color}') #output, prints the color given to the console
    #this program expects an input and cannot move forward without an input
    # you use both input() and print() in the assignement to show different results with our code    
